...
<?php
if (isset($wishesProducts) && count($wishesProducts) > 0): ?>
	<ul class="account__wishList">
		<?php
		foreach ($wishesProducts as $wishesProduct): ?>
			<li class="account__wishItem">
				<img src="/assets/images/productImages/<?= $wishesProduct->getCover()->getPath(
				) ?>" alt="product Image" class="account__wishImg">
				<p class="account__wishName"><?= SecurityService::safeString($wishesProduct->getTitle()) ?></p>
				<div class="account__wishFooter">
					<a href="/order/<?= $wishesProduct->getID() ?>/" data-id="<?= $wishesProduct->getID(
					) ?>" class="account__wishBuy">Buy now</a>
					<div class="product__footer_container">
						<p class="product__cost"><?= SecurityService::safeString($wishesProduct->getPrice()) ?>$</p>
					</div>
					<button onclick="removeFromWishList('<?= SecurityService::safeString(
						$wishesProduct->getTitle()
					) ?>', '<?= SecurityService::safeString($wishesProduct->getID()) ?>')" class="account__wishDelete">
